...
function collectPosts(data, config) {
  ...
  const rawPosts = getItemsOfType(data, 'post').filter(
  ...
  const posts = rawPosts.map((post) => ({
    ...
    frontmatter: {
      layout: 'post.11ty.js',
      author: 'Brett M. Nelson',
      title: getPostTitle(post),
      date: getPostDate(post),
      tags: ['post', ...getPostTags(post)],
      slug: getPostSlug(post),
      permalink: `/${getPostSlug(post)}/`,
      // coverImage: getPostCoverImageId(post),
    },
...
function getPostTags(post) {
  return post.category.map((c) => c._);
}
